...
sudo wget -O /usr/local/bin/pivestream.usb.py -q https://wesleymcginn.github.io/Pivestream/pivestream.usb.py

echo -e "\e[30;103;1mSetting script to start automatically on boot....\e[0m"
if crontab -l 2>/dev/null | grep -qF "@reboot python3 /usr/local/bin/pivestream.usb.py"; then
    echo "Pivestream USB is already set to run on boot."
else
    (crontab -l 2>/dev/null; echo "@reboot python3 /usr/local/bin/pivestream.usb.py") | crontab -
fi
